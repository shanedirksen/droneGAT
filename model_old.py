...
class CrossAttention(nn.Module):
    def __init__(self, input_channels=3, hidden_dim=128, output_dim=64, 
        ...
        self.cross_attn1 = CrossAttentionLayer(hidden_dim, num_heads=num_heads, dropout=dropout)
        self.cross_attn2 = CrossAttentionLayer(hidden_dim, num_heads=1, dropout=dropout)
    ...
    def message_passing(self, node_features, edge_index, relative_poses=None):
        """
        Apply cross attention message passing with spatial encoding
        ...
        if relative_poses is not None and relative_poses.shape[0] > 0:
            ...
            # First cross attention layer with encoded features
            x = F.elu(self.cross_attn1(node_features_encoded, edge_index))
        else:
            # Standard cross attention if no relative poses
            x = F.elu(self.cross_attn1(node_features, edge_index))
        
        x = self.dropout(x)
        
        # Second cross attention layer
        x = self.cross_attn2(x, edge_index)
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, node_features, edge_index):
        """
        Apply cross attention between connected nodes
        ...
        if edge_index.size(1) == 0:  # No edges
            return node_features  # Return original features if there are no connections
        
        # Prepare Q, K, V
        queries = self.query(node_features).view(num_nodes, self.num_heads, self.head_dim)
        keys = self.key(node_features).view(num_nodes, self.num_heads, self.head_dim)
        values = self.value(node_features).view(num_nodes, self.num_heads, self.head_dim)
        ...
        for node_idx in range(num_nodes):
            # Find neighbors (nodes that this node can attend to)
            # In a directed graph, we look at outgoing edges
            neighbors_idx = edge_index[1, edge_index[0] == node_idx]
            
            if len(neighbors_idx) == 0:
                output[node_idx] = node_features[node_idx]  # No neighbors, use original feature
                continue
                
            # Extract features for connected nodes
            node_query = queries[node_idx].unsqueeze(0)  # [1, num_heads, head_dim]
            neighbor_keys = keys[neighbors_idx]  # [num_neighbors, num_heads, head_dim]
            neighbor_values = values[neighbors_idx]  # [num_neighbors, num_heads, head_dim]
            
            # Calculate attention scores
            attn_scores = torch.bmm(
                node_query.transpose(0, 1),  # [num_heads, 1, head_dim]
                neighbor_keys.transpose(0, 1).transpose(1, 2)  # [num_heads, head_dim, num_neighbors]
            )  # [num_heads, 1, num_neighbors]
            ...
            attention_weights[node_idx] = attn_weights.detach()
            
            # Apply attention weights to values
            attn_output = torch.bmm(
                attn_weights,  # [num_heads, 1, num_neighbors]
                neighbor_values.transpose(0, 1)  # [num_heads, num_neighbors, head_dim]
            )  # [num_heads, 1, head_dim]
            
            # Concatenate heads and apply output projection
            attn_output = attn_output.transpose(0, 1).reshape(1, self.hidden_dim)  # [1, hidden_dim]
            output[node_idx] = self.out_proj(attn_output.squeeze(0))
